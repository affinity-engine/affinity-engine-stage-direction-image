...
test('Affinity Engine | stage | Directions | Image', function(assert) {
  ...
  visit('/image').then(() => {
    ...
    assert.ok(Ember.$(`${hook('affinity_engine_stage_direction_image')}:nth(3) img:nth(3)`).attr('src').match('affinity-engine/images/diy-default-nose.png'), 'it renders nose from array');
    ...
    assert.ok(Ember.$(`${hook('affinity_engine_stage_direction_image')}:nth(3) img:nth(4)`).attr('src').match('affinity-engine/images/diy-default-nose.png'), 'it still renders nose from array');
